    ...
    for epoch in tqdm(range(NUM_EPOCHS)):
        ...
        for batch in train_loader:
            ...
            for i, t in enumerate(all_tasks):
                ...
                reg_loss_data_main[t] = 0
                reg_loss_data_base[t] = 0
            ...
            for i, t in enumerate(tasks):
                ...
                if num_pos > C and num_neg > C:
                    ...
                    reg_loss_data_main[t] = task_reg.data.item()
                    if count > 0:
                        reg_main += task_reg
                    else:
                        reg_main = task_reg
                    rep_base_copy = rep_base.detach().clone()
                    rep_base_copy.requires_grad = False
                    mean_pos = torch.mean(rep_base_copy[mask_pos], axis=0, keepdim=True)
                    mean_neg = torch.mean(rep_base_copy[mask_neg], axis=0, keepdim=True)
                    num = num_pos + num_neg
                    if ord is not None:
                        task_reg = (num_pos * torch.linalg.matrix_norm(torch.cov(torch.transpose(rep_base_copy[mask_pos] - mean_pos, 0, 1)), ord=ord) + num_neg * torch.linalg.matrix_norm(torch.cov(torch.transpose(rep_base_copy[mask_neg] - mean_neg, 0, 1)), ord=ord)) / num
                    else:
                        task_reg = (num_pos * torch.sum(torch.square(rep_base_copy[mask_pos] - mean_pos)) / (num_pos - 1) + num_neg * torch.sum(torch.square(rep_base_copy[mask_neg] - mean_neg)) / (num_neg - 1)) / num
                    task_reg = task_reg / torch.square(torch.linalg.vector_norm(mean_pos - mean_neg, ord=2))
                    #task_reg = torch.reshape(task_reg, (1, 1))
                    #loss += model_base[t + "_weight"](task_reg)[0][0]
                    task_reg = rw * task_reg
                    reg_loss_data_base[t] = task_reg.data.item()
                    if count > 0:
                        reg_base += task_reg
                    else:
                        reg_base = task_reg
                    count += 1
            if count > 0:
                reg_main = reg_main / count
                reg_base = reg_base / count
                loss_main += reg_main
            loss_main.backward()
            optimizer_main.step()
            loss_base.backward()
            ...
            for t in tasks:
                ...
                writer.add_scalar('training_reg_loss_main_{}'.format(t), reg_loss_data_main[t], n_iter)
                writer.add_scalar('training_loss_base_{}'.format(t), loss_data_base[t], n_iter)
                writer.add_scalar('training_reg_loss_base_{}'.format(t), reg_loss_data_base[t], n_iter)
        for m in model_main:
            model_main[m].eval()
            model_base[m].eval()
            ...
            with torch.no_grad():
                ...
                for t in tasks:
                    ...
                    if num_pos > C and num_neg > C:
                        ...
                        task_reg = rw * task_reg
                        add_reg_main[t] = task_reg.item()
                        if count > 0:
                            reg_main += task_reg
                        else:
                            reg_main = task_reg
                        mean_pos = torch.mean(val_rep_base[mask_pos], axis=0, keepdim=True)
                        mean_neg = torch.mean(val_rep_base[mask_neg], axis=0, keepdim=True)
                        num = num_pos + num_neg
                        if ord is not None:
                            task_reg = (num_pos * torch.linalg.matrix_norm(torch.cov(torch.transpose(val_rep_base[mask_pos] - mean_pos, 0, 1)), ord=ord) + num_neg * torch.linalg.matrix_norm(torch.cov(torch.transpose(val_rep_base[mask_neg] - mean_neg, 0, 1)), ord=ord)) / num
                        else:
                            task_reg = (num_pos * torch.sum(torch.square(val_rep_base[mask_pos] - mean_pos)) / (num_pos - 1) + num_neg * torch.sum(torch.square(val_rep_base[mask_neg] - mean_neg)) / (num_neg - 1)) / num
                        task_reg = task_reg / torch.square(torch.linalg.vector_norm(mean_pos - mean_neg, ord=2))
                        task_reg = rw * task_reg
                        add_reg_base[t] = task_reg.item()
                        if count > 0:
                            reg_base += task_reg
                        else:
                            reg_base = task_reg
                ...
                if count > 0:
                    tot_loss_main['all'] += reg_main.item() / count 
                    tot_loss_base['all'] += reg_base.item() / count
                    reg_loss_main['all'] += reg_main.item() / count
                    reg_loss_base['all'] += reg_base.item() / count
                    for t in tasks:
                        add_reg_main[t] = add_reg_main[t] / count
                        add_reg_base[t] = add_reg_base[t] / count
                        reg_loss_main[t] += add_reg_main[t]
                        reg_loss_base[t] += add_reg_base[t]
                num_val_batches+=1
        ...
        for t in tasks:
            ...
            writer.add_scalar('validation_reg_loss_main_{}'.format(t), reg_loss_main[t]/num_val_batches, n_iter)
            writer.add_scalar('validation_reg_loss_base_{}'.format(t), reg_loss_base[t]/num_val_batches, n_iter)
        ...
        writer.add_scalar('validation_reg_loss_main', reg_loss_main['all']/len(val_dst), n_iter)
        writer.add_scalar('validation_reg_loss_base', reg_loss_base['all']/len(val_dst), n_iter)
