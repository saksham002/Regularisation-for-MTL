...
class CELEBA(data.Dataset):
    def __init__(self, root, split="train", is_transform=False, img_size=(32, 32), augmentations=None):
        ...
        self.files[self.split] = list(map(lambda x: '/'.join([base_path, x]), set(map(lambda x:x.split('/')[-1].split('\\')[-1], self.all_files)).intersection(set(selected_file_names))))
        self.labels[self.split] = list(map(lambda x: label_map[x], set(map(lambda x:x.split('/')[-1].split('\\')[-1], self.all_files)).intersection(set(selected_file_names))))
...
if __name__ == '__main__':
    ...
    bs = 4
    trainloader = data.DataLoader(dst, batch_size=bs, num_workers=1)
